...
import json
import config
from model.medusa import MedusaModel, MedusaHead
...
class LlamaHiddenStateExtractor:
    ...
    def __init__(self, model_path, device):
        self.device = device
        # Initialize the base model in eval mode
    ...
    def get_hidden_states(self, input_ids):
        ...
        for idx in range(batch_size):
            ...
            if hasattr(self.model, 'embed'):
                ...
                embeddings = self.model.embed(' '.join([str(t) for t in tokens]))
...
def load_and_preprocess_data(args):
    ...
    try:
        ...
        # Get text field based on dataset format
        if 'text' in dataset.column_names:
            ...
            text_col = next(col for col, type_info in dataset.features.items() 
                            if type_info.dtype == 'string')
    ...
    # Load tokenizer based on base model path
    try:
        # Attempt to load HF tokenizer first
        from pathlib import Path
        model_name = Path(args.base_model_path).stem
...
def train_medusa_heads(args):
    ...
    hidden_state_extractor = LlamaHiddenStateExtractor(args.base_model_path, device)
...
def parse_args():
    ...
    parser.add_argument("--dataset_split", type=str, default="wikitext-103-v1",
